...
test('help documents the fixed release source, commands, and the dsh CLI requirement', () => {
  ...
  assert.match(result.stdout, /github:shaomingbo\/dsh-visualization#v0\.3\.1/)
...
test('install initializes a fresh profile through the official CLI and is idempotent', async () => {
  ...
  try {
    const source = 'github:shaomingbo/dsh-visualization#v0.3.1'
    for (const args of [
      ['--source', source],
    ...
    assert.match(status.stdout, /github:shaomingbo\/dsh-visualization#v0\.3\.1/)
    const calls = await readLog(item.dshLog)
    assert.deepEqual(calls.filter(line => line.startsWith('plugin')), [
      'plugin --profile web add github:shaomingbo/dsh-visualization#v0.3.1 --ignore-scripts',
      'plugin --profile web add github:shaomingbo/dsh-visualization#v0.3.1 --ignore-scripts',
    ])
  } finally {
